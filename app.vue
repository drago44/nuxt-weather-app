<script setup lang="ts">
  ...
  const API_ID = '64e28f64b16b0f6cfeea0039ad3536da';

  const search = ref('Uzhgorod');
  const input = ref('');

  const { data: city, error } = useFetch(
    () => `${BASE_URL}data/2.5/weather?q=${search.value}&units=metric&appid=${API_ID}`,
  );
